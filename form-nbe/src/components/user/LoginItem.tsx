...
export interface LoginItemType {
  UserKey: React.FC<WrappedLoginItemProps>;
...
const LoginItem: React.FC<LoginItemProps> = (props) => {
  ...
  // This is written to prevent the introduction of restProps onChange, defaultValue, rules props tabUtil
  const {
    onChange,
    customProps,
    defaultValue,
        ...
        {({ getFieldValue }) => (
          <Row gutter={8}>
            ...
            <Col span={8}>
              ...
              >
                {timing ? `${count} second(s)` : 'get verification code'}
              </Button>
            </Col>
          </Row>
        )}
  ...
  if (type === 'Password') {
    return (
      <FormItem name={name} {...options}>
        <Input.Password {...customProps} {...otherProps} maxLength={maxLength} />
      </FormItem>
    );
  }
  return (
    <FormItem name={name} {...options}>
      <Input {...customProps} {...otherProps} maxLength={maxLength} />
    </FormItem>
  );
};
